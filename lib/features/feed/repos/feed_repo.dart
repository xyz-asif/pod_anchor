...
class FeedRepo {
  ...
  Future<PoemsPage> getHomeFeed({int limit = 20, String? before}) async {
    ...
    final response =
        await apiClient.get(ApiEndpoints.homeFeed, queryParameters: query);
  ...
  Future<PoemsPage> getExploreFeed(
      {int limit = 20, String? before, String? hashtag}) async {
    final query = <String, dynamic>{'limit': limit};
    if (before != null) query['before'] = before;
    if (hashtag != null && hashtag.isNotEmpty) query['hashtag'] = hashtag;
    final response =
        await apiClient.get(ApiEndpoints.exploreFeed, queryParameters: query);
  ...
  Future<PoemsPage> searchPoems(String q,
      {int limit = 20, String? before}) async {
    final query = <String, dynamic>{'q': q, 'limit': limit};
    if (before != null) query['before'] = before;
    final response =
        await apiClient.get(ApiEndpoints.searchPoems, queryParameters: query);
  ...
  Future<UserSearchPage> searchUsers(String q,
      {int limit = 20, int offset = 0}) async {
    final response = await apiClient.get(ApiEndpoints.searchUsers,
        queryParameters: {
          'q': q,
          'limit': limit,
          'offset': offset,
